...
function addNewPlayer() {
    ...
    if (password === "WhoSayCrackedWasTaken") {
        const username = prompt("Enter the new player's username:");
        if (username) {
            const gameMode = prompt("Enter the game mode (e.g., Axe, Sword, UHC, etc.):").toLowerCase();
            if (leaderboards[gameMode]) {
                let tier = prompt("Enter the tier (1-5):");
                tier = parseInt(tier);
                if (tier >= 1 && tier <= 5) {
                    const tierLabels = ["Combat Novice", "Combat Skilled", "Combat Pro", "Combat Ace", "Combat Master"];
                    const points = [200, 250, 280, 310, 340];
                    const newPlayer = {
                        player: username,
                        tier: tierLabels[tier - 1],
                        points: points[tier - 1],
                        skin: `https://mc-heads.net/avatar/${username}/32`
                    };
                    leaderboards[gameMode][`tier${tier}`].push(newPlayer);
                    updateLeaderboards();
                    alert(`${username} added to ${gameMode} Tier ${tier}!`);
                } else {
                    alert("Invalid tier! Please enter a number between 1 and 5.");
                }
            } else {
